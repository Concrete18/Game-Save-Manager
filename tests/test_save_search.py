...
class TestAppIDSearch:
    def test_get_appid(self):
        ...
        for test_value, answer in tests.items():
            assert search.get_appid(game=test_value) == answer
            sleep(0.5)


# @pytest.mark.skip
class TestSmartBrowse:
    def test_all_normal_searches(self):
        ...
        for game, actual_path in game_dict.items():
            ...
            print(found_path)
            assert actual_path.lower() in found_path.lower()
    ...
    #     search = SaveFinder(Game, cfg.custom_dirs, debug=False)
    #     # test
    #     found_path = search.find_save_location("Hitman 3")
